...
@section('content')
    <?php $supplies = \App\Supply::where('company_id',auth()->user()->company_id)->get(); ?>
    @if($supplies->count() <= 4)
                <div class="card">
                    @if(\Illuminate\Support\Facades\Session::get('supply'))
                        <div class="card-title green lighten-1 white-text" style="padding: 2%;">
                            {{\Illuminate\Support\Facades\Session::get('supply')}}
                        </div>
                    @else
                ...
                <div class="card">
                    <div class="card-content">
                        <form method="post" action="{{url('/sp/addSupply')}}" enctype="multipart/form-data">
                            {{csrf_field()}}
    ...
    @else
                <div class="card">
                    <div class="card-title red lighten-1 white-text" style="padding: 2%;">
                        you have reached maximum number of supply.
                    </div>
                </div>
        <!--  -->
    @endif
